					...
					<div class="card">
						<div class="card-header msg_head">
							<div class="d-flex bd-highlight">
								<div class="img_cont">
									<img src="/resources/popupSignin/images/profile.jpg" class="rounded-circle user_img">
								</div>
								<div class="user_info">
									<span>익명 채팅방</span>
								</div>
								<div class="video_cam" style="text-align:right;">
									<span id="back"><a href="/chat/chatList.chat"><i class="bi bi-arrow-left"></i></a></span>
...
<script>
		...
		client.connect({}, function() {
			...
			client.subscribe("/subscribe/chat/multi_room/" + '${room_id}', function(res) {
...
//======== 2. 메세지 송신(보내기) sender ===================
function send() {
	client.send("/publish/chat/multi_room", {}, JSON.stringify({
...
// ==== <<< 이 로직은 주최자와 관련된 로직 >> joinMeeting url 확인 분기점
	if('${organizer}'=='${id}'){
		...
		$('#invitation-icon').on('click', function(){
 			var result=confirm('단체 채팅방에 모두를 초대하시겠습니까?');
 	//================ 미팅 주최자================================================
 			if(result){
 				var userRoom_url="/subscribe/chat/multi_room/" + '${room_id}';
